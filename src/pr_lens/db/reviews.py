...
async def replayed_elsewhere(
    conn: asyncpg.Connection, batch: str, compare: Sequence[str] = ()
) -> set[tuple[str, int]]:
    ...
    Not any of `compare`'s either. Measuring a change to the reviewer means drafting the same
    ...
    this comparison, so the harness has to allow it. It takes a list rather than one name
    because the third measurement in a series has to leave out the first two, not just the
    one before it.
    """
    rows = await conn.fetch(
        "select repo, pr_number from review_runs "
        "where mode = 'replay' and batch <> all($1::text[])",
        [batch, *compare],
    )
