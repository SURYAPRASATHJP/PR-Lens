"""Ask each configured provider two questions before the pipeline depends on the answers.

The provider check in notes/phase-4-provider-check.md read the vendors' documentation. Two
things it could not settle from documentation are settled here, on the real endpoints, with
the real keys: whether a model honours a strict JSON schema, and whether a call sized to
CALL_TOKEN_BUDGET is accepted under an 8K-a-minute cap. The second probe also measures how
many characters a token really is for code, which is what CHARS_PER_TOKEN guesses at.

Each provider is asked on its own, not through failover, because the question is what each
one does. The result is a table in the job summary. A provider saying no is a finding, not
a failure, so this exits 0 unless no provider is configured at all.
"""
...
def _schema_verdict(content: str) -> str:
    try:
...
async def _ask(client: ChatClient, question: str, prompt: str) -> Probe:
    messages = [
    ...
    try:
        completion = await client.complete(
            messages, max_tokens=PROBE_COMPLETION_TOKENS, schema=SCHEMA
        )
    ...
    return Probe(
        ...
        completion.model,
        question,
        _schema_verdict(completion.content),
...
async def probe_all() -> list[Probe]:
    async with httpx.AsyncClient() as http:
        return [probe for client in configured(http) for probe in await check(client)]
